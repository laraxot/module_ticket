...
class JiraImport extends Page implements HasForms
{
    ...
    private ?bool $loadingProjects = null;

    private ?array $projects = null;

    public array $selected_projects;

    private ?bool $loadingTickets = null;

    private ?array $tickets = null;

    public array $selected_tickets;

    public array $data = [];

    public array $ticketsDataApi;
    ...
    public static function shouldRegisterNavigation(): bool
    {
        ...
        return $user->can('Import from Jira');
    }
                        ...
                        Step::make(__('Jira projects'))
                            ->schema([
                                Placeholder::make('hint')
                                    ...
                                    ->disableLabel()
                                    ->visible(fn (): bool => ! $this->loadingProjects && $this->projects)
                                ...
                                Placeholder::make('loading')
                                    ...
                                    ->disableLabel()
                                    ->visible(fn (): ?bool => $this->loadingProjects)
                                ...
                                CheckboxList::make('selected_projects')
                                    ...
                                    ->required()
                                    ->visible(fn (): ?array => $this->projects)
                                    ->options(function (): array {
                                        $list = [];
                                        if ($this->projects) {
                                            foreach ($this->projects as $project) {
                                                $list[$project->key] = new HtmlString(
                                                    "<div class='w-full flex flex-col gap-1'><div class='w-full flex items-center gap-1'><img src='".$project->avatarUrls->{'16x16'}."' class='rounded-full w-8 h-8 shadow' />"
                        ...
                        Step::make(__('Jira tickets'))
                            ->schema(function () {
                                ...
                                $fields[] = Placeholder::make('hint')
                                    ...
                                    ->disableLabel()
                                    ->visible(fn (): bool => ! $this->loadingTickets && $this->tickets)
                                    ->content(__('Choose your jira projects to import'));
                                ...
                                $fields[] = Placeholder::make('loading')
                                    ...
                                    ->disableLabel()
                                    ->visible(fn (): ?bool => $this->loadingTickets)
                                            ...
                                            if ($ticket['total'] > 0) {
                                                ...
                                                foreach ($ticket['issues'] as $issue) {
                                                    $fields[] = Checkbox::make('data.'.Str::slug($projectKey).'_'.Str::slug($issue['code']))
                                                        ->label(static fn (): HtmlString => new HtmlString(
                                                            "<div class='w-full flex flex-col gap-1'><div class='w-full flex items-center gap-1'>"
    ...
    {
        Assert::notNull($client = $this->connectToJira($this->host, $this->username, $this->token));
    ...
    public function updateJiraTickets(): void
    {
        $this->ticketsDataApi = [];
        Assert::notNull($client = $this->connectToJira($this->host, $this->username, $this->token));
        Assert::notNull($this->tickets = $this->getJiraTicketsByProject($client, $this->selected_projects));
        foreach ($this->tickets as $projectKey => $ticket) {
            foreach ($ticket['issues'] as $issue) {
